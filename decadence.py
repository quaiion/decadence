...
def graph_db_import() -> nx.Graph:
        return nx.read_gml(PATTERN + '_' + 'graph.gml')

def graph_db_save(graph: nx.Graph) -> None:
        nx.write_gml(graph, PATTERN + '_' + 'graph.gml')
...
def make_postponed_enhancements(graph: nx.Graph, human: bool) -> None:
        center = ''
        word_set = []
        after_file = open('to_be_decided.dat', 'r')
...
def remember_word_set(word_set: list) -> None:
        word_file = open('actual_word_set.dat', 'w')
        for word in word_set:
                word_file.write(word + '\n')
        word_file.close()

def retrieve_word_set() -> list:
        word_file = open('actual_word_set.dat', 'r')
        word_set = []
